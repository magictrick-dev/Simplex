#include <simplex/platform/darwin/darwin_filesystem.hpp>
...
static spx::hashed_sparse_map<int, spx::fs::darwin_file_handle> handles;

// --- Internal helpers --------------------------------------------------------

// NOTE(Chris): read()/write() cap a single transfer at a platform-defined maximum,
//              so positional I/O is chunked through this bound to stay portable.
static constexpr size_t darwin_max_io_chunk = 0x7FFFF000;

// NOTE(Chris): Newly created files get the conventional default mode; the umask
//              of the calling process narrows it as usual.
static constexpr mode_t darwin_default_create_mode = 0644;
...
static inline spx::fs::darwin_file_handle*
as_internal_handle(file_handle_t handle)
{
    return (spx::fs::darwin_file_handle*)handle;
}
...
// NOTE(Chris): _NSGetExecutablePath reports the required length when the buffer is
//              too small, so we grow until the full path fits. The reported path may
//              still contain symlinks or relative segments, so we resolve it through
//              realpath and fall back to the raw path when resolution is unavailable.
...
query_executable_path()
{
    ...
    while (true)
    {
        ...
        {
            // length now holds the required capacity; grow and retry.
        ...
        {
            spx::dynamic_string<char> result(resolved);
            simplex_memory_free(raw);
            return result;
        }

        spx::dynamic_string<char> result(raw);
        simplex_memory_free(raw);
        return result;
...
namespace spx::fs
{
                     ...
                     size_t *bytes_read)
    {
        ...
        *bytes_read = 0;
        ...
        while (total_read < file_size)
        {
            ...
            size_t to_read = (remaining > darwin_max_io_chunk) ? darwin_max_io_chunk : (size_t)remaining;
                      ...
                      size_t *bytes_written)
    {
        ...
        *bytes_written = 0;
        ...
        int native_handle = open(native_path.c_str(), O_WRONLY | O_CREAT | O_TRUNC, darwin_default_create_mode);
        if (native_handle < 0)
            return translate_errno(errno);
        ...
        while (total_written < buffer_size)
        {
            ...
            size_t to_write = (remaining > darwin_max_io_chunk) ? darwin_max_io_chunk : (size_t)remaining;
              ...
              PlatformFilesystemCreationFlag creation_flag)
    {
        ...
        *handle = NULL;
        ...
        int native_handle = open(native_path.c_str(), access_flags | creation_flags, darwin_default_create_mode);
        if (native_handle < 0)
        {
            int error_code = errno;
        ...
        darwin_file_handle &stored = handles.insert(native_handle, std::move(record));
    ...
    close_file(file_handle_t *handle)
    {
        ...
        darwin_file_handle *record = as_internal_handle(*handle);
